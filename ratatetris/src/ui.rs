...
fn render_grid(frame: &mut Frame, app: &App, area: Rect) {
    let box_size = cmp::min(area.height / GRID_Y as u16, area.width / GRID_X as u16); // would make them square (if using the right font)
    ...
    let rows = Layout::default()
        .direction(Direction::Vertical)
        .constraints(
            (0..GRID_Y).into_iter().map(|_| Constraint::Length(box_size))
        )
        .split(area);
    ...
    let row = Layout::default()
        ...
        .constraints(
            (0..GRID_X).into_iter().map(|_| Constraint::Length(box_size))
        )
        .split(rows[y]);

        for x in 0..GRID_X {
            let widget = Paragraph::new("").block(Block::default().borders(Borders::ALL));
            frame.render_widget(widget, row[x]);
        }
    }
